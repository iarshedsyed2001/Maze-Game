...
float px = 0.0, py = 175.0;  //  To store the Current Position

class maze
{
    public:
    int counted;                    //  To store Time Consumed
    int var = 0;                     //  Screen Variable
    clock_t start, ended, now;      //  TO store starting and ending time of the game
    vector< pair<float, float> > pt;  //  To store the visited positions
    ...
    //  Display the Finishing Position
    void finishPoint()
    {
        glColor3f(1.0,0.0,.0);
        glBegin(GL_POINTS);
    ...
    void pathTraced()
    {
        for(int j=0; j<pt.size(); j++)
        {
            ...
            glBegin(GL_POINTS);
            glVertex2f(pt[j].first, pt[j].second);
            glEnd();
        }
    }

    //  Print string in Glut window
    ...
    void winnerScreen()
    {
        ...
        printString(42,135,"CONGRATULATION YOU WON!!!");
        glColor3f(1.0,0.0,1.0);
        printString(11,100,"YOU HAVE SUCCEEDED IN FINDING OUT THE PATH");
        printString(35,60,"PRESS ESC TO RETURN TO MAIN MENU");
        printString(35,45,"PRESS 1 TO RESTART THE GAME");
        glFlush();
    }
    ...
    void startScreen()
    {
        ...
        printString(35,140,"WELCOME TO THE MAZE GAME");
        glColor3f(0.0,0.4,1.0);
        printString(50,100,"1.    START GAME");
        printString(50,80,"2.    HOW TO PLAY ??");
        printString(50,60,"3.    EXIT");
        glFlush();
    }
    ...
    void instructions()
    {
        glClear(GL_COLOR_BUFFER_BIT);
        glColor3f(0.0,0.4,1.0);
        printString(40,140,"----------INSTRUCTIONS----------");

        glColor3f(1,0,1);
        printString(-10,120,"->  USE ARROW KEYS TO MOVE THE POINTER");
        printString(-10,100,"->  REACH THE FINISH POINT BEFORE THE TIME RUNS OUT");
        printString(-10,80,"->  YELLOW COLOR REPRESENTS THE PATH TRACED");
        printString(-10,60,"->  IF YOU ARE LOST, PRESS 1 TO START FROM THE BEGINNING");
        printString(-10,40,"->  YOU HAVE 90 SECONDS TO FINISH THE GAME");
        printString(-10,20,"->  PRESS ESC TO RETURN TO MAIN MENU");
    ...
    void timeover()
    {
        glClear(GL_COLOR_BUFFER_BIT);
        glColor3f(0,0,1.0);
        printString(67,125,"TIMEOVER!!!!");
        glColor3f(0.0,0.5,0);
        printString(48,110,"YOU HAVE LOST THE GAME");
        printString(48,100,"BETTER LUCK NEXT TIME");
        printString(30,50,"->  PRESS ESC TO RETURN TO MAIN MENU");
        printString(30,40,"->  PRESS 1 TO RESTART THE GAME");
        glFlush();
    }

    //  Delay for smooth functionality
...
//  For the Movement of Current Pointer
void SpecialKey(int key, int x, int y)
{
  ...
  //  Push the position visited by the user
  m.pt.push_back({px,py});
}

//  To keep a check on Time and Status of the Game
void idle()
{
    ...
    {
        m.ended=clock();
        m.counted=(m.ended-m.start)/CLOCKS_PER_SEC;

        //  If Timeout display the message
        if(m.counted==90)
        {
            m.var=4;
        }
        //  Else if game Completed in time the display message for the Winner
        else if((m.counted<90) && ((px>=0 && px<=4) && (py>=162 && py<=168)))
        {
            m.var=5;
        }
    }
...
void display()
{
   ...
   glClear(GL_COLOR_BUFFER_BIT);
    ...
    {
		m.printString(-50,172, "START POINT");
		m.printString(-50,163, "FINISH POINT");

		//  Display the Time Remaining to Finish the Game
		glColor3f(0.0,0.0,1.0);
		m.printString(185,160, "TIME REMAINING : ");
		glColor3f(1,0,0);
	    char t[2];
		sprintf(t,"%d",90-m.counted);
        m.printString(245,160,t);

        //  Display the Path Traced and current, start and finish position
        m.pathTraced();
		m.currentPoint();
		m.startPoint();
		m.finishPoint();
		...
		//  Build the Maze
        glColor3f(0.0,0.0,0.0);     //  Black color Walls
		m.wall(-4,-4,0,-4,0,162,-4,162);
	    m.wall(-4,178,-4,184,184,184,184,178);
		m.wall(180,178,184,178,184,-4,180,-4);
		m.wall(180,0,180,-4,-4,-4,0,0);
		m.wall(18,8,42,8,42,12,18,12);
		m.wall(38,0,38,32,42,32,42,0);
		m.wall(42,28,42,32,52,32,52,28);
		m.wall(48,18,48,28,52,28,52,18);
		m.wall(58,8,58,82,62,82,62,8);
		m.wall(0,18,0,22,12,22,12,18);
		m.wall(8,8,12,8,12,18,8,18);
		m.wall(62,18,62,22,82,22,82,18);
		m.wall(48,8,48,12,82,12,82,8);
		m.wall(88,8,88,12,122,12,122,8);
		m.wall(138,0,138,12,142,12,142,0);
		m.wall(142,8,142,12,162,12,162,8);
		m.wall(128,8,132,8,132,32,128,32);
		m.wall(132,18,132,22,172,22,172,18);
		m.wall(168,18,168,8,172,8,172,22);
		m.wall(158,22,158,32,162,32,162,22);
		m.wall(118,8,118,32,122,32,122,8);
		m.wall(88,12,88,28,92,28,92,12);
		m.wall(78,28,78,32,102,32,102,28);
		m.wall(18,18,18,32,22,32,22,18);
        m.wall(22,18,22,22,32,22,32,18);
		m.wall(28,22,32,22,32,42,28,42);
		m.wall(32,38,32,42,72,42,72,38);
		m.wall(68,38,68,28,72,28,72,38);
		m.wall(78,48,78,28,82,28,82,48);
		m.wall(98,62,98,32,102,32,102,62);
		m.wall(68,52,68,48,92,48,92,52);
		m.wall(88,38,88,48,92,48,92,38);
		m.wall(108,82,108,18,112,18,112,82);
		m.wall(108,18,108,22,98,22,98,18);
		m.wall(180,28,180,32,168,32,168,28);
		m.wall(168,82,168,32,172,32,172,82);
		m.wall(168,78,168,82,158,82,158,78);
		m.wall(158,78,158,38,162,38,162,78);
		m.wall(158,38,158,42,118,42,118,38);
		m.wall(118,38,118,52,122,52,122,38);
		m.wall(122,52,122,48,132,48,132,52);
		m.wall(132,52,132,72,128,72,128,52);
		m.wall(138,42,138,72,142,72,142,42);
		m.wall(158,52,158,48,148,48,148,52);
		m.wall(142,58,142,62,152,62,152,58);
		m.wall(142,72,142,68,152,68,152,72);
		m.wall(112,62,112,58,122,58,122,62);
		m.wall(122,62,122,72,118,72,118,62);
		m.wall(8,28,8,42,12,42,12,28);
		m.wall(12,42,12,38,22,38,22,42);
		m.wall(18,42,18,48,22,48,22,42);
		m.wall(8,48,8,52,52,52,52,48);
		m.wall(8,52,8,62,12,62,12,52);
		m.wall(12,58,12,62,22,62,22,58);
		m.wall(18,92,18,62,22,62,22,92);
		m.wall(18,78,18,82,0,82,0,78);
		m.wall(18,102,18,98,62,98,62,102);
		m.wall(28,98,28,58,32,58,32,98);
		m.wall(32,58,32,62,52,62,52,58);
		m.wall(52,68,52,72,28,72,28,68);
		m.wall(62,92,62,88,32,88,32,92);
		m.wall(8,68,8,82,12,82,12,68);
		m.wall(68,48,68,62,72,62,72,48);
		m.wall(38,78,38,82,58,82,58,78);
		m.wall(62,62,62,58,92,58,92,62);
		m.wall(92,62,92,72,88,72,88,62);
		m.wall(108,68,108,72,92,72,92,68);
		m.wall(122,32,122,28,152,28,152,32);
		m.wall(62,72,62,68,72,68,72,72);
		m.wall(72,102,72,72,68,72,68,102);
		m.wall(72,102,72,98,78,98,78,102);
		m.wall(78,68,78,112,82,112,82,68);
		m.wall(82,82,82,78,102,78,102,82);
	    m.wall(78,108,78,112,48,112,48,108);
		m.wall(48,112,48,118,52,118,52,112);
		m.wall(38,122,38,118,102,118,102,122);
		m.wall(98,108,112,108,112,112,98,112);
        m.wall(8,88,12,88,12,112,8,112);
		m.wall(12,112,12,108,22,108,22,112);
		m.wall(22,112,22,122,18,122,18,112);
		m.wall(22,122,22,118,0,118,0,122);
		m.wall(8,122,8,132,12,132,12,122);
		m.wall(28,108,28,132,32,132,32,108);
		m.wall(28,128,28,132,12,132,12,128);
		m.wall(32,112,32,108,42,108,42,112);
		m.wall(42,108,42,102,38,102,38,108);
		m.wall(98,112,98,118,102,118,102,112);
		m.wall(112,132,112,128,58,128,58,132);
		m.wall(112,128,112,112,108,112,108,128);
		m.wall(58,152,58,128,62,128,62,152);
		m.wall(88,118,88,88,92,88,92,118);
		m.wall(118,88,118,78,122,78,122,88);
		m.wall(92,98,92,102,102,102,102,98);
		m.wall(92,92,92,88,122,88,122,92);
		m.wall(118,78,118,82,152,82,152,78);
		m.wall(152,82,148,82,148,88,152,88);
		m.wall(152,92,152,88,128,88,128,92);
		m.wall(128,88,128,98,132,98,132,92);
		m.wall(108,98,108,102,142,102,142,98);
		m.wall(118,102,118,162,122,162,122,102);
		m.wall(122,108,122,112,132,112,132,108);
		m.wall(132,112,132,122,128,122,128,112);
		m.wall(142,122,142,118,132,118,132,122);
		m.wall(180,88,180,92,158,92,158,88);
		m.wall(158,92,158,112,162,112,162,92);
		m.wall(172,112,172,108,162,108,162,112);
		m.wall(172,112,172,108,158,108,158,112);
		m.wall(152,122,152,98,148,98,148,122);
		m.wall(148,112,148,108,138,108,138,112);
		m.wall(152,118,152,122,172,122,172,118);
		m.wall(168,162,168,122,172,122,172,162);
		m.wall(168,142,168,138,158,138,158,142);
		m.wall(168,152,168,148,158,148,158,152);
        m.wall(148,102,148,98,172,98,172,102);
		m.wall(-4,172,-4,168,72,168,72,172);
		m.wall(172,162,172,158,148,158,148,162);
		m.wall(152,162,152,132,148,132,148,162);
		m.wall(180,172,180,168,148,168,148,172);
		m.wall(142,172,142,168,108,168,108,172);
		m.wall(78,172,78,168,102,168,102,172);
		m.wall(102,172,102,162,98,162,98,168);
		m.wall(122,162,122,158,78,158,78,162);
		m.wall(72,172,72,158,68,158,68,172);
		m.wall(162,132,162,128,128,128,128,132);
		m.wall(128,132,128,172,132,172,132,132);
		m.wall(142,172,142,168,108,168,108,172);
		m.wall(142,138,142,162,138,162,138,138);
		m.wall(138,158,138,162,132,162,132,158);
	    m.wall(118,142,118,138,88,138,88,142);
		m.wall(112,152,112,148,68,148,68,152);
		m.wall(72,148,72,132,68,132,68,148);
		m.wall(82,142,82,138,72,138,72,142);
		m.wall(42,122,42,142,38,142,38,122);
		m.wall(38,142,38,138,18,138,18,142);
	    m.wall(22,142,22,148,18,148,18,142);
		m.wall(18,152,18,148,48,148,48,152);
		m.wall(48,128,52,128,52,158,48,158);
		m.wall(62,162,62,158,18,158,18,162);
		m.wall(22,162,22,168,18,168,18,162);
		m.wall(72,172,72,168,0,168,0,172);
		m.wall(72,158,72,168,68,168,68,158);
		m.wall(12,162,12,148,8,148,8,162);
		m.wall(8,162,8,158,0,158,0,162);
		glutPostRedisplay();
	}
...
{
    //  Start the game if 1 is Pressed on Start Screen, Timeover Screen, or Winning Screen
    if(key=='1' && (m.var==0 || m.var==4 || m.var==5))
    {
        //  Set the current position to Starting Position Before Starting the Game
        px = 0.0;
        py = 175.0;

        m.pt.clear();       //  Before every New Game Clear the vector storing the Path visited in the Previous Game
        m.start=clock();    //  Store the Starting time of the Game
        m.var = 1;          //  Start the game
        glutPostRedisplay();
    }

    //  Display the Instructions
    else if(m.var==0 && key=='2')
        m.var=2;

    //  Quit the Game
    else if(m.var==0 && key=='3')
        m.var=3;

    //  Return to The Start Screen if ESC is pressed
    else if(key == 27)
        m.var=0;


    //  Go back to Starting Point if lost during playing the Game
    if(key=='1' && m.var==1)
    {
       px = 0.0;
       py = 175.0;
